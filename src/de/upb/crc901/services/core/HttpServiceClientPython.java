...
public class HttpServiceClientPython {

	private static Logger logger = LoggerFactory.getLogger(HttpServiceClientPython.class.getName());

	private final OntologicalTypeMarshallingSystem otms;

	public HttpServiceClientPython(OntologicalTypeMarshallingSystem otms) {
		super();
		this.otms = otms;
	}
	...
	public ServiceCompositionResult callServiceOperation(OperationInvocation call, SequentialComposition coreography,
		...
		/*
		 * writes the inputs parameter name as the index of an array "inputs",
		 * followed by the serialized representation of the corresponding
		 * object.
		 */
		...
		/*
		 * Traversing over the Jackson Json tree to get a
	...
	public ServiceCompositionResult invokeServiceComposition(SequentialComposition composition,
		...
		for (OperationInvocation opInv : composition) {
			...
			// TODO Why are the outputs added here?
			availableInputs.addAll(
					opInv.getOutputMapping().values().stream().map(p -> p.getName()).collect(Collectors.toList()));
		}

		/*
		 * Get first service call and invoke it with the rest of the composition
		 * as a choreography.
		 */
