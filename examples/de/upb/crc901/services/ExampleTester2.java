...
import de.upb.crc901.services.core.HttpServiceClientPython;
...
public class ExampleTester2 {
	...
	private final static int JAVA_PORT = 8000;
	private final static int PYTHON_PORT = 8002;

	private HttpServiceServer javaServer;
	private HttpServiceServerPython pythonServer;

	private SequentialComposition composition;
	private SequentialCompositionSerializer sqs;
	private HttpServiceClientPython client;
	...
	public void init() throws Exception {
		/* start server */
		pythonServer = new HttpServiceServerPython(PYTHON_PORT, "testrsc/conf/operations.conf", "testrsc/conf/types.conf");

		/* read in composition */
		sqs = new SequentialCompositionSerializer();
		composition = sqs.readComposition(FileUtil.readFileAsList("testrsc/composition.txt"));

		client = new HttpServiceClientPython(otms);
	}
	...
	public void testSciKit() throws Exception {
		...
		ServiceCompositionResult constructCall = client.callServiceOperation("127.0.0.1:" + PYTHON_PORT + "/" + "SVC" + "::__construct");
		String serviceId =	constructCall.get("out").asText();
	...
	public void testClassifier() throws Exception {
		...
		String serviceId = client
				.callServiceOperation("127.0.0.1:" + PYTHON_PORT + "/" + c.getClass().getName() + "::__construct")
				.get("out").asText();
		client.callServiceOperation(serviceId + "::buildClassifier", split.get(0));
